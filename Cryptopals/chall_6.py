...
file = open('chall_6.txt')
ciphertext = base64.b64decode(file.read())

def get_eng_score(input_bytes):
    ...
    return sum([char_freqs.get(chr(byte), 0) for byte in input_bytes.lower()])
...
def break_repeating_key_xor(ciphertext):
    ...
    return max(possible_plaintext, key=lambda x: get_eng_score(x[0]))
